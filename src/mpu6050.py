...
class MPU(object):
    def __init__(self, i2c=None, intr=None, rate=None,
                 address=None):
        
        self.bus = i2c if i2c is not None else print("* MPU ERROR: please provide a i2c object")
        ...
        # self.init_pins() # i2c is already given
        # self.init_i2c() # already done in MAIN
        self.init_device()
    ...
    def identify(self):
        print('* MPU: identifying i2c device')
        val = self.read_byte(MPU6050_RA_WHO_AM_I)
        print("* MPU: MPU6050_RA_WHO_AM_I: ", val)
        if val != MPU6050_ADDRESS_AD0_LOW:
            raise OSError(" MPU ERROR: No mpu6050 at address {}".format(self.address))
    ...
    def wait_for_stable(self, numsamples=10):
        print('* MPU: waiting for gyros to stabilize')
        ...
        now = time.time()
        print('* MPU: gyros stable after {:0.2f} seconds'.format(now-time_start))
        ...
        try:
            ...
            for passno in range(20):
                ...
                check = [0 if expected[i] is None else expected[i] - avg[i]
                       for i in range(7)]
                print('* MPU: - pass {}: {}'.format(passno, check))
            ...
            print('* MPU WARNING: ! calibration failed')
            return
